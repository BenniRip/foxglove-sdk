...
public:
  // _service is kept alive (and declared before _subscriber, so destroyed after it) because the
  ...
  bool drain(foxglove::RawChannel& channel, const rclcpp::Logger& logger) override {
    while (true) {
      auto sample = _subscriber.receive();
      if (!sample.has_value()) {
        ...
        RCLCPP_WARN(logger, "iox2 source: receive error on '%s', disabling", _serviceName.c_str());
        return false;
      }
      if (!sample->has_value()) {
        break;  // queue drained
      ...
      if (err != foxglove::FoxgloveError::Ok && !_warnedLogError) {
        _warnedLogError = true;
        RCLCPP_WARN(logger, "iox2 source: log failed on topic '%s': %s", _topic.c_str(),
...
private:
  ...
  Service _service;
  Sub _subscriber;
  [[maybe_unused]] bool _warnedBadHeader = false;  // unused in the headerless instantiation
  bool _warnedLogError = false;
};
...
std::vector<Iox2Source::Config> Iox2Source::loadParameters() {
  ...
  _node.declare_parameter("iox2.use_message_header", true);
  ...
  _useHeader = _node.get_parameter("iox2.use_message_header").as_bool();
...
void Iox2Source::setupSubscribers(const foxglove::Context& context,
  ...
  for (const auto& cfg : configs) {
    const auto bfbs = loadBinaryFile(_schemaDir / cfg.bfbsFile);
    ...
    auto subscription =
      _useHeader ? openSubscription<Iox2MessageHeader, true>(*_iox2Node, cfg.serviceName, cfg.topic)
                 : openSubscription<void, false>(*_iox2Node, cfg.serviceName, cfg.topic);

    _entries.push_back(Entry{std::move(channelResult.value()), std::move(subscription)});

    RCLCPP_INFO(_node.get_logger(), "iox2 source: '%s' -> Foxglove '%s' (%s)%s",
                cfg.serviceName.c_str(), cfg.topic.c_str(), cfg.schemaName.c_str(),
                _useHeader ? "" : " [headerless]");
  }
}
...
void Iox2Source::pollOnce() {
  for (auto& entry : _entries) {
    ...
    if (!entry.subscription->drain(entry.channel, _node.get_logger())) {
